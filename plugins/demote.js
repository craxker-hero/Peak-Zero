const demoteHandler = async (m, { conn, isAdmin, isBotAdmin, isOwner, isROwner, participants, groupMetadata, usedPrefix, command }) => {
  ...
  try {
    ...
    m.reply(`*❮⚠️❯ @${userToDemote.split('@')[0]} ya no es administrador*`, null, { mentions: [userToDemote] });
  } catch (error) {
    console.error(error);
    m.reply('*❮⚠️❯ No se pudo quitar los privilegios de admin. Verifica los permisos del bot.*');
  }
};
